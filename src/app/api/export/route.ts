  ...
  try {
    ...
    const htmlContent = `
      <html>
        ...
        .prose h2 {
            font-size: 1.5rem;
            margin-top: 2.5rem; /* dari mt-10 */
            margin-bottom: 0.5rem; /* dari mb-2 */
        }

        .prose p {
            margin-top: 0.75rem; /* dari my-3 */
        ...
        <body class="prose prose-sm lg:prose-base prose-p:my-3 prose-ul:my-3 prose-h2:mt-10 prose-h2:mb-2 leading-relaxed prose-p:text-justify prose-li:text-justify prose-hr:border-gray-300 dark:prose-hr:border-gray-700 dark:text-gray-200 prose-strong:dark:text-white prose-headings:dark:text-white">${marked(
          markdown
        )}</body>
      </html>
    `;
    ...
    const pdfBuffer = await page.pdf({
      format: "A4",
      displayHeaderFooter: true, // Aktifkan header/footer
      headerTemplate: `
